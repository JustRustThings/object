...
impl<'data> DataDirectories<'data> {
    ...
    pub fn export_table<R: ReadRef<'data>>(
        ...
        let file_offset_of_directory = data_dir.file_offset_range(sections)?.0;
        ExportTable::parse(
            file_data,
    ...
    pub fn max_directory_file_offset<R: ReadRef<'data>>(&self, file_size: Option<u64>, data: R, section_table: &'data SectionTable) -> Option<u64> {
        let mut max = None;
        ...
        for (dir_index, directory) in self.enumerate() {
            ...
            match (file_offset as u64)
                .checked_add(directory.size.get(LE) as u64)
            {
                None => {
